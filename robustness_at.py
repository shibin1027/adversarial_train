...
from robust_pkg.adv_attack import fgsm, pgd, cw
from robust_pkg.data_loader import clean_loader_cifar, adv_loader_data
from robust_pkg.inference_tools import inference
...
args.model_checkpoint = './checkpoint/resnet50_advtrain_final.pth'
...
def craft_adv_samples(data_loader, model, args, attack_method):
    ...
    for bi, (inputs, targets) in enumerate(data_loader):
        ...
        if attack_method == 'fgsm':
            crafted, l2 = fgsm(inputs, targets, model, num_cls=args.num_cls, eps=args.eps)
        elif attack_method == 'pgd10':
            crafted, l2 = pgd(inputs, targets, model, num_cls=args.num_cls, eps=args.eps, iters=10)
        elif attack_method == 'pgd20':
            crafted, l2 = pgd(inputs, targets, model, num_cls=args.num_cls, eps=args.eps, iters=20)
        elif attack_method == 'pgd50':
            crafted, l2 = pgd(inputs, targets, model, num_cls=args.num_cls, eps=args.eps, iters=50)
        elif attack_method == 'cw10':
            crafted, l2 = cw(inputs, targets, model, num_cls=args.num_cls, eps=args.eps, iters=10)
        elif attack_method == 'cw20':
            crafted, l2 = cw(inputs, targets, model, num_cls=args.num_cls, eps=args.eps, iters=20)
        else:
...
def main():
    ...
    inference(model, clean_loader, args, note='natural')
    ...
    for attack_method in ['fgsm', 'pgd10', 'pgd20', 'cw10']:
        ...
        inference(model, adv_loader, args, note=attack_method)
