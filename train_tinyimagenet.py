...
from models import ResNet50, Feat1_ResNet50_TINY
...
sys.stdout = Logger(os.path.join(args.save_dir, args.ckpt_name+'_tinyimagenet_sp30.txt'))
...
if args.ckpt_name == 'resnet50':
    ...
    model = Feat1_ResNet50_TINY(num_classes=200)
...
def robust_inference(epoch):
    ...
    torch.save(state, os.path.join('checkpoint', args.ckpt_name + '_tinyimagenet_sp30_final.pth'))
    if rsr > best_acc:
        print('Saving current best model..')
        if not os.path.isdir('checkpoint'):
            os.mkdir('checkpoint')
        torch.save(state, os.path.join('checkpoint', args.ckpt_name+'_tinyimagenet_sp30_best.pth'))
        best_acc = rsr
