...
sys.stdout = Logger(os.path.join(args.save_dir, args.ckpt_name+'_svhn.txt'))
...
test_dataset = datasets.SVHN('data', split='test', download=True, transform=transforms.Compose([transforms.ToTensor()]))
...
def robust_inference(epoch):
    ...
    torch.save(state, os.path.join('checkpoint', args.ckpt_name + '_svhn_final.pth'))
    if rsr > best_acc:
        print('Saving current best model..')
        if not os.path.isdir('checkpoint'):
            os.mkdir('checkpoint')
        torch.save(state, os.path.join('checkpoint', args.ckpt_name+'_svhn_best.pth'))
        best_acc = rsr
